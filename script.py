...
def create_pipe():
  random_pipes_height = random.choice(pipes_height)
  top_pipe = pipe_surface.get_rect(midtop = (700, random_pipes_height))
  bottom_pipe = pipe_surface.get_rect(midbottom = (700, random_pipes_height - 300))
...
def randomize_assets():
  # background
  random_bg = random.choice(bg_choices)
  bg_surface = pygame.image.load(random_bg).convert()
  bg_surface = pygame.transform.scale2x(bg_surface)
  # pipes
  random_pipe = random.choice(pipe_choices)
  pipe_surface = pygame.image.load(random_pipe).convert()
  pipe_surface = pygame.transform.scale2x(pipe_surface)
  # bird
  random_bird = random.choice(bird_choices)
  if random_bird == 'sprites/bluebird-midflap.png':
  ...
  elif random_bird == 'sprites/redbird-midflap.png':
    bird_downflap = pygame.transform.scale2x(pygame.image.load('sprites/redbird-downflap.png').convert_alpha())
    bird_midflap = pygame.transform.scale2x(pygame.image.load('sprites/redbird-midflap.png').convert_alpha())
    bird_upflap = pygame.transform.scale2x(pygame.image.load('sprites/redbird-upflap.png').convert_alpha())
  elif random_bird == 'sprites/yellowbird-midflap.png':
...
bg_choices = ['sprites/background-day.png', 'sprites/background-night.png']
bird_choices = ['sprites/bluebird-midflap.png', 'sprites/redbird-midflap.png', 'sprites/yellowbird-midflap.png']  
pipe_choices = ['sprites/pipe-green.png', 'sprites/pipe-red.png']
...
while True:
  ...
  if game_active:
    ...
    score += 0.007
    if score > score_sound_countdown:
      score_sound.play()
      score_sound_countdown += 1
